            ...
            try:
                ...
                for route,route_link in zip(route,route_link): # Iterating through each bus routes
                    ...
                    try: # Scraping data
                        ...
                        bus1=wait.until(EC.visibility_of_all_elements_located((By.XPATH, '//div[contains(@class,"travels lh-24")]')))
                        bus2=wait.until(EC.visibility_of_all_elements_located((By.XPATH, '//div[contains(@class,"dp-loc l-color w-wrap")]')))
                        bus3=wait.until(EC.visibility_of_all_elements_located((By.XPATH, '//div[contains(@class,"bp-loc l-color w-wrap")]')))
                        bus4=wait.until(EC.visibility_of_all_elements_located((By.XPATH, '//div[contains(@class,"bus-type")]')))
                        bus5=wait.until(EC.visibility_of_all_elements_located((By.XPATH, '//span[contains(@class,"f-19 f-bold")]|//span[contains(@class,"f-bold f-19")]')))
                        bus6=wait.until(EC.visibility_of_all_elements_located((By.XPATH, '//div[contains(@class,"seat-left")]')))
                        bus7=wait.until(EC.visibility_of_all_elements_located((By.XPATH, '//div[@class="dp-time f-19 d-color f-bold"]')))
                        bus8=wait.until(EC.visibility_of_all_elements_located((By.XPATH, '//div[@class="bp-time f-19 d-color disp-Inline"]')))
                        bus9=wait.until(EC.visibility_of_all_elements_located((By.XPATH, '//div[@class="dur l-color lh-24"]')))
                        bus10=wait.until(EC.visibility_of_all_elements_located((By.XPATH, '//div[@class="rating-sec lh-24"]')))
                        # Storing the scraped date to list                
                        def App(num1, num2):
                            for b in num1:
                                num2.append(b.text)
                        App(bus1,busname)
                        ...
                        pass
                    df=pd.DataFrame(list(zip(govt_bus_name,route_name,route_link1,bus_start_point,bus_end_point,busname,bustype,departing_time,duration,reaching_time,star_rating,price,seats_available)),columns=['govt_bus_name','route_name','route_link','bus_start_point','bus_end_point','busname','bustype','departing_time','duration','reaching_time','star_rating','price','seats_available'])
                    replacement_values = {
