...
# Data Scraping from Red Bus Website
    ...
    st.header('DATA SCRAPING PROCESS STRATED')    
    ...
    for a in range(0,12):
        ...
        st.write(f'No. {a}: Data Scraping for the Bus :',govt_bus[a])
            ...
            try:
                ...
                for route,route_link in zip(route,route_link): # Iterating through each bus routes
                    ...
                    try: # Scraping data
                        ...
                        bus1=wait.until(EC.visibility_of_all_elements_located((By.XPATH, '//div[contains(@class,"travels lh-24")]'))) # Busname
                        bus2=wait.until(EC.visibility_of_all_elements_located((By.XPATH, '//div[contains(@class,"dp-loc l-color w-wrap")]'))) # bus_start_point
                        bus3=wait.until(EC.visibility_of_all_elements_located((By.XPATH, '//div[contains(@class,"bp-loc l-color w-wrap")]'))) # bus_end_point
                        bus4=wait.until(EC.visibility_of_all_elements_located((By.XPATH, '//div[contains(@class,"bus-type")]'))) # bustype
                        bus5=wait.until(EC.visibility_of_all_elements_located((By.XPATH, '//span[contains(@class,"f-19 f-bold")]|//span[contains(@class,"f-bold f-19")]'))) # price
                        bus6=wait.until(EC.visibility_of_all_elements_located((By.XPATH, '//div[contains(@class,"seat-left")]'))) # seats_available
                        bus7=wait.until(EC.visibility_of_all_elements_located((By.XPATH, '//div[@class="dp-time f-19 d-color f-bold"]'))) # departing_time
                        bus8=wait.until(EC.visibility_of_all_elements_located((By.XPATH, '//div[@class="bp-time f-19 d-color disp-Inline"]'))) # reaching_time
                        bus9=wait.until(EC.visibility_of_all_elements_located((By.XPATH, '//div[@class="dur l-color lh-24"]'))) # duration
                        bus10=wait.until(EC.visibility_of_all_elements_located((By.XPATH, '//div[@class="rating-sec lh-24"]'))) # star_rating
                        # Storing the scraped date Global to list                
                        def App(num1, num2):
                            for b in num1:
                                num2.append(b.text)
                        App(bus1,busname)
                        App(bus2,bus_start_point)
                        App(bus3,bus_end_point)
                        App(bus4,bustype)
                        App(bus5,price)
                        App(bus7,departing_time)
                        App(bus8,reaching_time)
                        App(bus9,duration)
                        App(bus10,star_rating)
                        # Local list to store the Scraped data
                        ...
                        for c in bus1:
                            route_name.append("{0}".format(route,route_link))
                            govt_bus_name.append(govt_bus[a])
                            route_link1.append("{1}".format(route,route_link))
                    ...
                    df=pd.DataFrame(list(zip(govt_bus_name,route_name,route_link1,col2,col3,col1,col4,col7,col9,col8,col10,col5,col11)),columns=['govt_bus_name','route_name','route_link','bus_start_point','bus_end_point','busname','bustype','departing_time','duration','reaching_time','star_rating','price','seats_available'])
    ...
    # Storing values in a df from scraped CSV file
    current_dir = os.getcwd()
